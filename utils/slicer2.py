...
class Slicer:
    ...
    # @timeit
    def slice(self, waveform, waveform_vocal):
        # check if the waveform and waveform_vocal have the same length
        if len(waveform.shape) != len(waveform_vocal.shape):
            raise ValueError('The waveform and waveform_vocal must have the same length')
        
        if len(waveform_vocal.shape) > 1:
            samples = waveform_vocal.mean(axis=0)
        else:
            samples = waveform_vocal
...
def main():
    ...
    parser.add_argument('vocal_audio', type=str, help='The vocal audio extracted from the audio to be sliced')
