...
def combine_spectrograms(specs, param):
    ...
    for d in range(1, bands_n + 1):
        ...
        spec_c[:, offset:offset+h, :l] = specs[d][:, param['band'][d]['crop_start']:param['band'][d]['crop_stop'], :l]
        offset += h
    ...
    # lowpass fiter
    if param['pre_filter_start'] > 0: # and param['band'][bands_n]['res_type'] in ['scipy', 'polyphase']:   
        if bands_n == 1:
...
def cmb_spectrogram_to_wave(spec_m, param, extra_bins_h=None, extra_bins=None):
    ...
    for d in range(1, bands_n + 1):
        ...
        if d == bands_n: # higher
            ...
            if bands_n == 1:
                wave = spectrogram_to_wave(spec_s, bp['hl'], param['mid_side'], param['mid_side_b2'], param['reverse'])
            else:
                wave = np.add(wave, spectrogram_to_wave(spec_s, bp['hl'], param['mid_side'], param['mid_side_b2'], param['reverse']))
        else:
            sr = param['band'][d+1]['sr']
            if d == 1: # lower
                spec_s = fft_lp_filter(spec_s, bp['lpf_start'], bp['lpf_stop'])
                wave = librosa.resample(spectrogram_to_wave(spec_s, bp['hl'], param['mid_side'], param['mid_side_b2'], param['reverse']), bp['sr'], sr, res_type=wav_resolution)
            else: # mid
                spec_s = fft_hp_filter(spec_s, bp['hpf_start'], bp['hpf_stop'] - 1)
                spec_s = fft_lp_filter(spec_s, bp['lpf_start'], bp['lpf_stop'])
                wave2 = np.add(wave, spectrogram_to_wave(spec_s, bp['hl'], param['mid_side'], param['mid_side_b2'], param['reverse']))
...
def mirroring(a, spec_m, input_high_end, param):
    if 'mirroring' == a:
        mirror = np.flip(np.abs(spec_m[:, param['pre_filter_start']-10-input_high_end.shape[1]:param['pre_filter_start']-10, :]), 1)
